...
class RawHTTPLoggingMiddleware:
    ...
    def __init__(self, app: Callable, logger: RawHTTPLogger = None):
        self.app = app
    ...
    async def __call__(self, scope: dict, receive: Callable, send: Callable) -> None:
        ...
        if hasattr(self.logger, 'config') and self.logger.config:
            ...
            if any(path.startswith(exclude) for exclude in self.logger.config.exclude_paths):
                return await self.app(scope, receive, send)
            
            # Then check include_paths (if specified, only log included paths)
            if self.logger.config.include_paths:
                if not any(path.startswith(include) for include in self.logger.config.include_paths):
                    return await self.app(scope, receive, send)
    ...
    async def _log_request_headers(self, scope: dict, request_id: str) -> None:
        ...
        exclude_headers = []
        if hasattr(self.logger, 'config') and self.logger.config:
            exclude_headers = [h.lower().encode() for h in self.logger.config.exclude_headers]
