...
class RequestTracerConfig(BaseModel):
    ...
    enabled: bool = Field(
        default=True, 
        description="Enable or disable the request tracer plugin"
    )
    ...
    json_logs_enabled: bool = Field(
        default=True,
        description="Enable structured JSON logging to files"
    )
    
    # Raw HTTP logging (from raw_http_logger)
    raw_http_enabled: bool = Field(
        default=False,
        description="Enable raw HTTP protocol logging"
    )
    
    # Directory configuration
    log_dir: str = Field(
        default="/tmp/ccproxy/traces",
        description="Base directory for all trace logs"
    )
    request_log_dir: Optional[str] = Field(
        default=None,
        description="Override directory for structured JSON logs (defaults to log_dir)"
    )
    raw_log_dir: Optional[str] = Field(
        default=None,
    ...
    include_paths: list[str] = Field(
        ...
        description="If specified, only trace these paths"
    )
    
    # Privacy & security
    exclude_headers: list[str] = Field(
        default_factory=lambda: ["authorization", "x-api-key", "cookie", "x-auth-token"],
        description="Headers to redact in raw logs"
    )
    redact_sensitive: bool = Field(
        default=True,
        description="Redact sensitive data in structured logs"
    )
    ...
    log_client_request: bool = Field(
        default=True,
        description="Log client requests"
    )
    log_client_response: bool = Field(
        default=True,
        description="Log client responses"
    )
    log_provider_request: bool = Field(
        default=True,
        description="Log provider requests"
    )
    log_provider_response: bool = Field(
        default=True,
        description="Log provider responses"
    )
    
    # Streaming configuration
    log_streaming_chunks: bool = Field(
        default=False,
        description="Log individual streaming chunks (verbose)"
    )
    ...
    class Config:
        ...
        env_prefix = "PLUGINS__REQUESTTRACERCONFIG__"
