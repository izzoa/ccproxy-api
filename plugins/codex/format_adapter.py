...
class CodexFormatAdapter(APIAdapter):
    ...
    async def adapt_request(self, request_data: dict[str, Any]) -> dict[str, Any]:
        ...
        if "messages" in request_data:
            ...
            response_request = self._response_adapter.chat_to_response_request(request_data)
            ...
            logger.info("codex_request_conversion", 
    ...
    async def adapt_response(self, response_data: dict[str, Any]) -> dict[str, Any]:
        ...
        if self._is_response_api_format(response_data):
            ...
            chat_response = self._response_adapter.response_to_chat_completion(response_data)
    ...
    async def adapt_stream(  # type: ignore[override,misc]
        ...
        async for event in stream:
            logger.debug("processing_response_event", event_type=event.get("type"), message_id=message_id)
            ...
            chunk = self._convert_response_event_to_chat_delta(event, message_id, created)
            if chunk:
