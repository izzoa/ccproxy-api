    ...
    def discover_plugins(self) -> dict[str, Path]:
        ...
        for item in self.plugins_dir.iterdir():
            if item.is_dir() and not item.name.startswith("_"):
                # Check for v2_plugin.py file (new plugin format)
                v2_plugin_file = item / "v2_plugin.py"
                if v2_plugin_file.exists():
                    self.discovered_plugins[item.name] = v2_plugin_file
                    logger.debug(
                        "plugin_discovered", name=item.name, path=str(v2_plugin_file)
                    )
    ...
    def load_plugin_factory(self, name: str) -> PluginFactory | None:
        ...
        try:
            ...
            spec = importlib.util.spec_from_file_location(
                f"plugins.{name}.v2_plugin", plugin_path
            )
