...
class OAuthTemplates:
    ...
    @classmethod
    def invalid_state(
        cls, provider: OAuthProvider = OAuthProvider.GENERIC
    ) -> HTMLResponse:
        """Generate response for invalid state parameter.

        Args:
            provider: OAuth provider name

        Returns:
            HTML response for invalid state
        """
        return cls.error(
    ...
    def missing_code(
        ...
        """Generate response for missing authorization code.

        Args:
            provider: OAuth provider name

        Returns:
            HTML response for missing code
        """
        return cls.error(
            error_message="No authorization code was received.",
            title="Missing Authorization Code",
            error_detail=f"The {provider.value} server did not provide an authorization code. Please try again.",
            status_code=400,
        )

    @classmethod
    def token_exchange_failed(
        cls,
        error_detail: str | None = None,
        provider: OAuthProvider = OAuthProvider.GENERIC,
    ) -> HTMLResponse:
        """Generate response for token exchange failure.
        ...
        Returns:
            HTML response for token exchange failure
        """
        return cls.error(
            error_message=f"Failed to exchange authorization code for {provider.value} tokens.",
    ...
    def storage_error(
        ...
        """Generate response for credential storage errors.
        ...
        Returns:
            HTML response for storage errors
        """
        return cls.error(
            error_message="Failed to save your credentials.",
            title="Storage Error",
            error_detail=error_detail
            or "There was a problem saving your authentication credentials. Please check file permissions and try again.",
            status_code=500,
        )
