...
class SubprocessSecurity:
    ...
    def _validate_working_directory(self) -> None:
        ...
        if self.working_directory:
            ...
            if not path.is_dir():
                raise ValueError(f"Working directory '{self.working_directory}' is not a directory")
            if not os.access(self.working_directory, os.R_OK | os.X_OK):
                raise ValueError(f"Working directory '{self.working_directory}' is not accessible")
    ...
    def _drop_privileges(self) -> None:
        ...
        if self.group:
            ...
            logger.debug(f"Dropped group privileges to {self.group} (gid: {group_info.gr_gid})")

        if self.user:
            user_info = pwd.getpwnam(self.user)
            os.setuid(user_info.pw_uid)
            logger.debug(f"Dropped user privileges to {self.user} (uid: {user_info.pw_uid})")
    ...
    def secure_run(self, cmd: list[str], **kwargs) -> subprocess.CompletedProcess:
        ...
        logger.debug(f"Security settings - User: {self.user}, Group: {self.group}, CWD: {self.working_directory}")
...
def get_default_claude_security(working_directory: str | None = None) -> SubprocessSecurity:
    """
